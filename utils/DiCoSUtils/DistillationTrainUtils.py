...
def train_DiCoS(config:DiCoSConfig, schema:MultiWoZSchema, model:nn.DataParallel):
    ...
    list_param = [
        {'params': model.module.dicos.encoder.parameters(), 'lr':config.encoder_learning_rate},
        {'params': [param for name, param in model.module.dicos.named_parameters() if 'encoder' not in name and 'bias' not in name], 'lr':config.basic_learning_rate, 'weight_decay':0.01},
        {'params': [param for name, param in model.module.dicos.named_parameters() if 'encoder' not in name and 'bias' in name], 'lr':config.basic_learning_rate, 'weight_decay':0}
    ]
    ...
    loss_distillation_f = nn.MSELoss(reduction='sum')
    ...
    for epoch in range(100):
        for i, data in enumerate(trainset):
            ...
            score, selectedScore, startProb, endProb, slotValueProb, selectedHistory_sentencesTokens, gold_attentionMap, student_attentionMap = model.forward(current_sentencesTokens, current_attentionMask, current_segmentEmbedding, 
            ...
            # 混合蒸馏loss
            loss_distillation = loss_distillation_f(student_attentionMap, gold_attentionMap)/config.batch_size
            ...
            # 保存感兴趣的梯度信息
            select_grad = model.module.dicos.multiPrespectiveSelector.hiddenState2score.weight.grad.clone().detach().cpu()
            cls_grad = model.module.dicos.SVG.fcForEachSlot.grad.clone().detach().cpu()
            start_grad = model.module.dicos.SVG.fc_start.weight.grad.clone().detach().cpu()
            end_grad = model.module.dicos.SVG.fc_end.weight.grad.clone().detach().cpu()
            
            # 保存参数
            list_train_selectedScore += selectedScore.clone().detach().cpu().tolist()
            ...
            if (total_batch) % 100 == 0:
                ...
                # 模型权重
                config.tbWriter.add_histogram('weight/cls', model.module.dicos.SVG.fcForEachSlot, global_step=total_batch)
                config.tbWriter.add_histogram('weight/startExt', model.module.dicos.SVG.fc_start.weight, global_step=total_batch)
                config.tbWriter.add_histogram('weight/endExt', model.module.dicos.SVG.fc_end.weight, global_step=total_batch)
                config.tbWriter.add_histogram('weight/selector', model.module.dicos.multiPrespectiveSelector.hiddenState2score.weight, global_step=total_batch)

                # 分数分布
                config.tbWriter.add_histogram('score/score', score.clone().detach().cpu(),  global_step=total_batch)
                config.tbWriter.add_histogram('score/selectedScore', np.array(list_train_selectedScore),  global_step=total_batch)
            ...
            if total_batch % config.eval_step == 0:
                ...
                if test_acc >= best_test_acc:
                    ...
                    torch.save(model.module.dicos.state_dict(), config.save_path)
                    improve = '*'
